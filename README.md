...
顶栏选择“电机测试”即可进入独立的 CAN FD 电机台架页面。默认配置面向当前单电机环境：

- ZLG 设备类型 `41`（USBCANFD-200U）、设备索引 `0`、物理通道 `CAN2`（SDK `CH1`）
- 电机页使用资料包 `二次开发库V1.21\x64\ControlCANFD.dll`；不要选择 IAP 页使用的 32 位 `zlgcan.dll`
- 仲裁域 `1 Mbps`、数据域 `5 Mbps`、CAN FD + BRS
- 电机 ID `0x01`、位置广播 ID `0x300`
- 位置心跳 `50 Hz`、测试时长 `24 小时`

推荐流程：打开 CAN FD，先读取电机状态；收到当前位置后点击“安全启动保持”。工具默认采用反馈位置作为首次目标，依次设置控制源、失能、故障复位、启动位置心跳并使能。结束时点击“安全停止”，工具会先失能，再停止心跳。长时间测试可先点击“开始记录 CSV”，反馈数据会流式写入文件。

位置心跳由独立线程发送，避免界面刷新阻塞控制周期。测试过程中如出现关节状态 3、状态反馈中断超过 500 ms、位置偏差超过 0.10 rad、心跳间隔超过 120 ms，或使能后 2 秒内未确认运行状态，工具会立即停止心跳并连续发送两次失能。故障排查阶段先做短时间空载测试，确认稳定后再延长测试时长。

需要观察故障后的完整现象时，可勾选“故障复现模式”。该模式下状态 3、位置偏差和反馈超时只告警、不自动失能；CAN 发送失败仍会停机，紧急失能始终可用。启用时会要求二次确认。

电机页在内存中保存最近 200,000 帧原始 TX/RX。点击“开始记录（含已有）”会先将点击前的缓存写入 CSV，再持续追加之后的所有原始帧，周期位置心跳也包含在内。“清空缓存”只清除内存和界面，不会截断已经打开的 CSV 文件。
